...
void pcalorimeters(TH2F* hcalHist)
{
  ...
  const double pi = 4. * atan2(1.,1.);
   ...
   //TH2F * hcalHist = new TH2F("h2", "h2", 24, -3.14*0.8, 3.14*0.8,
   //			      48, -3.14 , 3.14);

   // TH2F * hcalHist = new TH2F("h2", "h2", 24,
   // 			      -1. * ( 90-36.82) * 2 *pi/180., 1. * ( 90-36.82) * 2 *pi/180.,
   // 			      48, -1*pi , pi);
   // hcalHist->Fill(0.,0.);
   // hcalHist->Fill(0.,0.7);
   // hcalHist->Fill(1.,0., 2);
   ...
   auto calo3d = MakeCalo3D(data, slot);
...
TEveCalo3D* MakeCalo3D(TEveCaloData* data, TEveWindowSlot* slot)
{
   ...
   calo3d->SetBarrelRadius(190.5);
   calo3d->SetEndCapPos(292.);
   s->AddElement(calo3d);
